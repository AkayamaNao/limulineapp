...
rich_menu_to_create = RichMenu(
    ...
    areas=[RichMenuArea(
        ...
        action=PostbackAction(data="{'action':'rich_order'}",text='注文')),
        RichMenuArea(
            bounds=RichMenuBounds(x=625, y=0, width=625, height=843),
            action=PostbackAction(data="{'action':'rich_cancel'}",text='キャンセル')),
        RichMenuArea(
            bounds=RichMenuBounds(x=1250, y=0, width=625, height=843),
            action=PostbackAction(data="{'action':'rich_check'}",text='注文確認')),
        RichMenuArea(
            bounds=RichMenuBounds(x=1875, y=0, width=625, height=843),
            action=PostbackAction(data="{'action':'rich_menu'}",text='メニュー'))
    ]
)
